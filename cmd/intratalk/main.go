...
func main() {
	// Command line flags
	server := flag.String("server", "192.168.36.2:64738", "the server to connect to")
	username := flag.String("username", "rpi101", "the username of the client")
	password := flag.String("password", "", "the password of the server")
	insecure := flag.Bool("insecure", true, "skip server certificate verification")
	certificate := flag.String("certificate", "", "PEM encoded certificate and private key")
	channel := flag.String("channel", "Main Channel", "mumble channel to join by default")
